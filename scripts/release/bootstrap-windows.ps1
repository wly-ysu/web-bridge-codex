...
try {
    ...
    & (Join-Path $root "scripts\release\install-windows.ps1") -Repository $Repository -DownloadUrl $DownloadUrl -NonInteractive:$NonInteractive -AcceptAiProfile:$AcceptAiProfile -SkipBrowserLaunch:$SkipBrowserLaunch
    exit $LASTEXITCODE
} finally {
