...
has_mcp=false; [ -f "$MCP_CONFIG" ] && grep -q '^\[mcp_servers\.pro_bridge_codex\]' "$MCP_CONFIG" && has_mcp=true
has_rule=false; [ -f "$AGENTS" ] && grep -q 'pro_bridge_codex:web-first:start' "$AGENTS" && has_rule=true
printf '%s\n' \
  "UNIX_DOCTOR" \
